---
title: "EBS Pollock RTMB-ADMB Implementation"
execute:
  echo: true
  warning: false
  message: false
...
params:
  ...
  sparsenuts_file: "analysis/output/sparsenuts/rtmb_admb_sparsenuts_default.rds"
...
if (is.na(pollock_root) || !nzchar(pollock_root)) {
  pollock_root <- Sys.getenv("POLLOCK_BASE", unset = NA_character_)
}
if (is.na(pollock_root) || !nzchar(pollock_root)) {
  parent_candidate <- dirname(rtmb_root)
  if (file.exists(file.path(parent_candidate, "admb", "runs", "for_rtmb"))) {
    pollock_root <- parent_candidate
  }
}
if (is.na(pollock_root) || !nzchar(pollock_root)) {
  stop("Set POLLOCK_ROOT to a directory containing admb/runs/for_rtmb/.")
}
...
osa_output_file <- rtmb_file("analysis", "output", "osa", "rtmb_admb_osa_residuals.rds")
...
bridge_files <- tibble(
  file = c(rtmb_metadata$admb_rep, rtmb_metadata$admb_par, pollock_file("admb", "runs", "for_rtmb", "pm.tpl")),
  ...
  modified = as.POSIXct(file.info(file)$mtime)
)
...
saveRDS(
  list(report = rtmb_result$rtmb, metadata = list(model = "rtmb_admb")),
...
The RTMB bridge can now write a Standard Projection Model input directory for use with `spmR`. This report writes the current saved RTMB output to `r projection_dir`, using the same SPM setup structure as the ADMB bridge projection directory while replacing the species assessment input file with an RTMB-derived `pm.prj`.
...
spmr_input <- if (file.exists(file.path(spmr_projection$output_dir, "spm.dat")) &&
                  requireNamespace("spmR", quietly = TRUE)) {
  tryCatch(
    spmR::dat2list(file.path(spmr_projection$output_dir, "spm.dat")),
...
if (!is.null(spmr_input)) {
  ...
  tibble(
    ...
    value = c(spmr_projection$output_dir, "spmR is not installed in the render environment")
  ) |>
    gt_report() |>
    tab_header(title = "Generated SPM Setup")
}
...
if (nrow(spmr_alt2_detail) > 0) {
  ...
  tibble(note = paste("No Alternative 2 fixed-catch SPM output found at", spmr_alt2_detail_file)) |>
    gt_report()
}
```
...
if (nrow(spmr_detail) > 0) {
  ...
  tibble(note = paste("No SPM projection detail output found at", spmr_detail_file)) |>
    gt_report()
}
```

```{r}
#| label: fig-spmr-projection-ssb-catch
#| fig-cap: "Mean SPM projected spawning biomass and catch by alternative, read from spm_detail.csv when available."
#| fig-width: 9
#| fig-height: 5
if (nrow(spmr_detail) > 0) {
  spmr_detail |>
    ...
    pivot_longer(c(SSB, Catch), names_to = "quantity", values_to = "value") |>
    ggplot(aes(x = Year, y = value, color = factor(Alt))) +
    geom_line(linewidth = 0.8) +
    facet_wrap(~quantity, scales = "free_y", ncol = 1) +
    scale_y_continuous(limits = c(0, NA), labels = scales::label_comma()) +
    labs(x = "Year", y = "Mean value", color = "Alternative", title = "SPM Projection Means")
} else {
  plot.new()
  text(0.5, 0.5, "Run SPM to create spm_detail.csv for projection plots.", cex = 0.9)
}
```

```{r}
#| label: fig-spmr-projection-alt2-trajectories
#| fig-cap: "Alternative 2 SPM projected spawning biomass and catch means with 10 simulated trajectories overlaid as thin gray lines."
#| fig-width: 9
#| fig-height: 5
if (nrow(spmr_detail) > 0) {
  alt2_detail <- spmr_detail |>
    filter(Alt == 2)

  alt2_sims <- sort(unique(alt2_detail$Sim))
  alt2_sims <- alt2_sims[seq(1, length(alt2_sims), length.out = min(10, length(alt2_sims)))] |>
    round() |>
    unique()

  alt2_sim_plot <- alt2_detail |>
    filter(Sim %in% alt2_sims) |>
    select(Sim, Year, SSB, Catch) |>
    pivot_longer(c(SSB, Catch), names_to = "quantity", values_to = "value")

  alt2_mean_plot <- alt2_detail |>
    group_by(Year) |>
    summarize(
      SSB = mean(SSB, na.rm = TRUE),
      Catch = mean(Catch, na.rm = TRUE),
  ...
  ggplot() +
    geom_line(
      data = alt2_sim_plot,
      aes(x = Year, y = value, group = interaction(quantity, Sim)),
      color = "gray65",
      linewidth = 0.25,
      alpha = 0.75
    ) +
    ...
    facet_wrap(~quantity, scales = "free_y", ncol = 1) +
    scale_y_continuous(limits = c(0, NA), labels = scales::label_comma()) +
    labs(x = "Year", y = "Value", title = "Alternative 2 Projection Means and Simulated Trajectories")
} else {
  plot.new()
  text(0.5, 0.5, "Run SPM to create spm_detail.csv for Alternative 2 projection plots.", cex = 0.9)
}
```
...
model_status_tbl <- tibble(
  ...
  value = c(
    ...
    rtmb_metadata$admb_rep,
    rtmb_metadata$admb_par,
...
if (nrow(retro_diagnostics) > 0) {
  ...
  tibble(note = paste("No retrospective output found at", retro_file)) |>
    gt_report()
}
```
...
repro_tbl <- tibble(
  ...
  value = c(
    ...
    model_file,
    sparsenuts_file,
    pollock_file("admb", "runs", "for_rtmb"),
    rtmb_root
  )
)
...
sparsenuts_run_metadata <- attr(sparsenuts_fit, "rtmb_admb_sparsenuts") %||% list()
...
sparsenuts_figures <- list(
  pairs_slow = file.path(sparsenuts_fig_dir, "rtmb_admb_sparsenuts_pairs_slow.png"),
  marginals_slow = file.path(sparsenuts_fig_dir, "rtmb_admb_sparsenuts_marginals_slow.png"),
  sampler = file.path(sparsenuts_fig_dir, "rtmb_admb_sparsenuts_sampler_params.png"),
  q = file.path(sparsenuts_fig_dir, "rtmb_admb_sparsenuts_Q.png"),
  uncertainties = file.path(sparsenuts_fig_dir, "rtmb_admb_sparsenuts_uncertainties.png")
)
