...
### Sparse-metric pilot comparison

The eight-chain Form 2 run used `metric = "diag"`, so it did not use the sparse Cholesky preconditioner even though SparseNUTS constructed an 804 by 804 sparse precision-matrix object. That matrix contained 30.44% structural zeros (69.56% nonzero entries), so its potential computational advantage was not obvious in advance. A matched pilot compared `diag` and `sparse` metrics using the same Form 2 model, saved precision matrix, initial values, seed, two serial chains, and 500 iterations per chain (250 warmup). Reusing the precision matrix isolates the sampling transformation from its one-time construction cost.

```{r}
#| label: tbl-sparsenuts-metric-pilot
#| tbl-cap: "Matched SparseNUTS pilot comparing diagonal and sparse metrics for the hierarchical Form 2 model. Times are means per serial chain. Speedup is diagonal total time divided by the metric-specific total time. The pilot is intended to compare computation and is too short for inference."
sparsenuts_metric_pilot <- readr::read_csv(
  rtmb_file("reporting", "data", "sparsenuts_metric_pilot.csv"),
  show_col_types = FALSE
)

sparsenuts_metric_pilot |>
  transmute(
    Metric = metric,
    Chains = chains,
    Iterations = iterations,
    `Mean warmup (s)` = mean_warmup_seconds,
    `Mean sampling (s)` = mean_sampling_seconds,
    `Mean total (s)` = mean_total_seconds,
    `Total-time speedup` = speedup_total_vs_diag,
  ...
  fmt_number(
    columns = c(
      `Mean warmup (s)`, `Mean sampling (s)`, `Mean total (s)`,
      `Minimum bulk ESS`, `Median bulk ESS`
    ),
    decimals = 1
  ) |>
  fmt_number(columns = `Total-time speedup`, decimals = 2, pattern = "{x}x") |>
  fmt_number(columns = c(`Divergences (%)`, `Maximum R-hat`), decimals = 3) |>
  tab_header(title = "SparseNUTS Metric Pilot")
```

Sparse preconditioning reduced mean total chain time from 42.4 to 14.2 seconds, a 2.99-fold speedup, and improved the pilot's worst R-hat and minimum bulk effective sample size. It also produced 9 post-warmup divergences (1.8%), whereas the diagonal pilot produced none. Thus the model demonstrably uses sparse factorization when `metric = "sparse"`, and doing so was substantially faster in this pilot, but the sparse configuration is not yet suitable for inference. A longer tuned run should address the divergences and compare effective samples per second before replacing the current diagonal-metric result. The original full Form 2 run spent about 474 seconds constructing the precision matrix and its statistics; that one-time setup cost was deliberately excluded from both pilot timings by reusing the same matrix.
