...
if (!is.null(sparsenuts_fit) && !is.null(sparsenuts_fit$monitor)) {
  ...
  diag_tbl <- tibble(
    diagnostic = c(
      "Post-warmup draws", "Divergences", "Divergent transitions (%)",
      "Maximum R-hat", "Minimum bulk ESS", "Median bulk ESS",
      "Minimum tail ESS", "Maximum-treedepth transitions (%)"
    ),
    value = c(
      post_warmup_total,
      round(post_warmup_total * divergence_percent / 100),
      divergence_percent,
...
The default run provides good overall chain mixing, but any post-warmup divergences mean that the affected posterior regions were not traversed reliably. These draws are therefore useful for diagnosing the base model and selecting the next sampler adjustment, but they should not yet replace the deterministic uncertainty estimates in assessment decisions.
