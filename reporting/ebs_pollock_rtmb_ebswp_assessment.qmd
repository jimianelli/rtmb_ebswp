...
params:
  ...
  retro_file: "analysis/output/retro_9_peel.rds"
...
retro_file <- rtmb_file(params$retro_file %||% "analysis/output/retro_9_peel.rds")
...
osa_residual_method <- if (isTRUE(osa_precomputed_available)) {
  paste0(
    ...
    " output generated from the accepted RTMB base model"
  )
...
# Bridging ADMB to RTMB {#sec-admb-rtmb-bridge}

This is the same fixed-parameter bridge demonstrated in the September 2025
[EBS pollock development report](https://noaa-afsc.github.io/EBS_pollock/doc/Sept_2025.html#bridging-admb-to-rtmb). The modified ADMB model's maximum-likelihood parameter estimates are injected into RTMB, and the two implementations are evaluated at that identical parameter point. This isolates the code translation from optimization behavior or later model alternatives.

The refreshed `base.rds` is the authoritative starting point for every result below. Its configuration label, bridge statistics, and file identity are carried into the projections, retrospectives, and SparseNUTS metadata so those analyses cannot silently use a different selectivity form or an older base run.

```{r}
#| label: tbl-bridge-summary
#| tbl-cap: "Summary of the fixed-parameter ADMB-to-RTMB bridge, recalculated from the September 2025 configuration. The absolute NLL difference is on the negative-log-likelihood scale; the key-output difference is the largest absolute percentage difference among state, prediction, selectivity, composition, and likelihood quantities."
...
tibble(
  ...
  tab_header(title = "September 2025 ADMB–RTMB Bridge")
```

The total negative log-likelihoods differ by only `r sprintf("%.9f", bridge_metrics$absolute_total_nll_difference)`. The maximum difference among key outputs is `r sprintf("%.9f%%", bridge_metrics$maximum_key_percent_difference)`, and the largest absolute RTMB gradient at the ADMB estimates is `r format(bridge_metrics$maximum_absolute_gradient, scientific = TRUE, digits = 7)`. These reproduce the near-perfect September bridge and pass the repository's automated bridge tolerances.

```{r}
#| label: tbl-downstream-lineage
#| tbl-cap: "Base-model lineage for downstream analyses. Matching checksums confirm that projections, retrospectives, and SparseNUTS were regenerated from the same refreshed September 2025 bridge output."
...
mcmc_lineage <- readr::read_csv(
  rtmb_file("reporting", "data", "sparsenuts_base_run_summary.csv"),
  show_col_types = FALSE
)
lineage_tbl <- tibble(
  analysis = c(
    "Authoritative base bridge", "Nine-peel retrospective",
    "Seven-scenario Tier 3 projections", "Alternative 2 fixed-catch projections",
    "SparseNUTS MCMC"
  ),
  base_md5 = c(
    current_base_md5,
    ...
    mcmc_lineage$base_md5[1] %||% NA_character_
  )
) |>
...
lineage_tbl |>
  gt_report() |>
  tab_header(title = "Common September-Bridge Lineage")
```
...
The age-composition diagnostics follow the updated [afscOSA guidance](https://noaa-afsc.github.io/afscOSA/) incorporated through [afscOSA pull request 10](https://github.com/noaa-afsc/afscOSA/pull/10). The diagnostics were regenerated from the accepted September 2025 bridge base model rather than from a selectivity sensitivity. In this render, the method is `r osa_residual_method`.

The four rows in @fig-osa-age-diagnostics should be read together. The aggregate composition is the first check for systematic lack of fit, such as a selectivity pattern that consistently misses particular ages. The Q-Q plot assesses whether OSA residuals behave approximately as standard normal values and reports the SDNR and lower and upper 2.5% quantiles with their expected intervals. The OSA and Pearson bubble plots then help locate temporal, age-specific, or cohort patterns. A statistic outside its interval is a clue to investigate, not a stand-alone rule for accepting or rejecting an assessment configuration.

```{r}
#| label: tbl-osa-residual-status
#| tbl-cap: "Source and software checks for the updated OSA diagnostics. The base-model checksum ties these diagnostics to the same accepted RTMB object used elsewhere in this report. The ATS 2020 placeholder was excluded because it contains no rounded multinomial observations and therefore supplies no information to an OSA calculation."
tibble(
  item = c(
    "afscOSA version",
...
#| fig-cap: "Age-composition diagnostics produced with afscOSA for the fishery, bottom trawl survey (BTS), and acoustic-trawl survey (ATS). Top row: observed aggregate proportions (blue bars), expected proportions (red points and line), and 95% intervals for data expected from the fitted model (red vertical lines); the intervals condition on fitted parameters and are neither confidence intervals nor posterior predictive intervals. ISS and ESS are the aggregate input and effective sample sizes. Second row: OSA residual Q-Q plots with SDNR and 2.5% tail quantiles; parentheses give their expected 95% intervals. Third and fourth rows: OSA and Pearson residual bubbles by year and age, with red positive and blue negative residuals and bubble area increasing with absolute magnitude. Bubble sizes are standardized across fleets; two Pearson residuals greater than 6 were truncated to 6 for display only. The diagnostics should be interpreted jointly and do not constitute a stand-alone model acceptance or rejection test."
...
The aggregate panels show generally close observed and expected patterns, with localized departures that deserve attention rather than a wholesale failure of the composition model. The BTS SDNR (0.96) lies within its displayed expected interval. Fishery (0.87) and ATS (0.82) have narrower OSA residual distributions than standard normal expectations; this can reflect composition weighting, residual dependence, or other model structure and should be considered alongside the aggregate and bubble patterns. The fishery upper tail is also lighter than expected, while the other tail estimates are close to or within their displayed intervals. These results identify where follow-up work may be useful, but they do not by themselves determine the preferred assessment model.
...
The RTMB retrospective script was run for peel 0 (the full 2024 model) and nine retrospective peels terminating in 2023 through 2015. Each peel removes observations and weight-at-age inputs after its terminal year, shortens the corresponding annual parameter vectors, and starts from the preceding fitted peel. The BTS index has no 2020 observation in these inputs; the peeling code retains that data gap rather than treating 2020 as an observed BTS year.
...
All nine peels completed and produced trajectory estimates. Nine of the ten fits including peel 0 ended with convergence code 0. Peel 2 ended its tighter restart with code 1, but its maximum gradient was `r scales::number(retro_diagnostics$max_gradient[retro_diagnostics$peel == 2], accuracy = 0.000001)`, below the report's 0.001 gradient criterion. The largest maximum gradient across all fits was `r scales::number(max(retro_diagnostics$max_gradient, na.rm = TRUE), accuracy = 0.0001)`. Mohn's rho was `r scales::number(retro_mohn$rho[retro_mohn$quantity == "SSB"], accuracy = 0.001)` for spawning biomass and `r scales::number(retro_mohn$rho[retro_mohn$quantity == "Recruitment"], accuracy = 0.001)` for recruitment. Both are positive, and the spawning-biomass value is driven particularly by the deeper peels terminating in 2015--2020. These results indicate a substantial positive retrospective pattern and should be considered when evaluating stability and uncertainty.
...
This analysis applies the current `SparseNUTS` package to the **accepted base RTMB model** (`fishery_sel_form = 0`), not to the experimental Form 2 double-logistic model. The call leaves chains, cores, sampling iterations, warmup, initialization, and metric selection at the package defaults, but increases `adapt_delta` to 0.95 to reduce divergent transitions. The model name and `globals = list(data = model_data)` are also supplied. Exporting that RTMB data object lets the package rebuild the objective in its parallel R workers, so the package-default chains can run concurrently.

The completed fit is saved outside the rendered document. Every render reads that fit and rebuilds all MCMC tables and figures, preventing diagnostics from an older run from remaining in the report. The reproducible runner is `R/run_sparsenuts_default.R`.
...
if (param_is_true(params$run_sparsenuts) &&
  ...
  if (!sparsenuts_available) {
    ...
    run_status <- system2(
      ...
      rtmb_file("R", "run_sparsenuts_default.R")
    )
...
sparsenuts_status <- tibble(
  ...
  value = c(
    sparsenuts_run_metadata$model %||% "base RTMB model",
    ...
    "SparseNUTS::sample_snuts(obj, globals = list(data = model_data))",
...
#| tbl-cap: "Diagnostics for the current package-default SparseNUTS run of the base RTMB model. R-hat values near 1 and larger effective sample sizes indicate good between-chain mixing. Divergences identify transitions that did not adequately explore the posterior geometry and should be resolved before using the draws for final inference."
...
diag_tbl |>
  gt_report() |>
  fmt_number(columns = value, decimals = 3) |>
  tab_header(title = "SparseNUTS Base-Model Diagnostics")
```

With `adapt_delta = 0.95`, the run produced `r warmup_divergences` divergences during adaptation and **`r sampling_divergences` divergences among the `r format(post_warmup_total, big.mark = ",")` retained transitions**. Maximum R-hat was `r sprintf("%.3f", max(sparsenuts_fit$monitor$rhat, na.rm = TRUE))`, and minimum bulk effective sample size was `r format(round(min(sparsenuts_fit$monitor$ess_bulk, na.rm = TRUE)), big.mark = ",")`. `r if (sampling_divergences == 0) "The retained draws pass the reported divergence and chain-mixing checks." else paste0("The chain-mixing statistics are good, but ", sampling_divergences, " retained divergence", if (sampling_divergences == 1) " remains" else "s remain", "; its location should be checked before treating the posterior as final.")` Substantive posterior checks and comparison with deterministic uncertainty remain appropriate before using the draws in assessment decisions.
