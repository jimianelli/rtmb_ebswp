...
subtitle: 'Working paper on the corrected full-age 2024 "PM" model bridge and subsequent RTMB developments'
author:
  - name: James Ianelli
    corresponding: true
...
abstract: |
  ...
  to R Template Model Builder (RTMB), the fixed-parameter bridge used to test
  implementation equivalence, and subsequent developments made possible by
  the RTMB framework. The report brings the bridge, model fits, fishery and
  survey selectivity work, projections, retrospective analysis, and
  SparseNUTS diagnostics into one reproducible account. The accepted ADMB
  model remains the management reference; the RTMB results presented here
  support implementation testing and evaluation of candidate refinements.
execute:
  echo: false
  warning: false
  message: false
...
The custom ADMB pollock model has been translated into RTMB and tested at the
same parameter point as a corrected ADMB bridge run. The corrected case
...
fixed-parameter comparison separates code translation from optimization and
later model development. The saved corrected RTMB bridge supplies a common
starting point for model-fit summaries, projections, retrospective peels, and
sensitivities in this report.

The principal developments are:

- an R implementation of the ADMB population dynamics and likelihood;
- explicit object-by-object and likelihood-component bridge checks;
- selectable fishery-selectivity forms, including hierarchical
  time-varying and two-dimensional autoregressive alternatives;
- RTMB-based retrospective and uncertainty workflows;
- a reproducible lineage from the ADMB bridge files to saved RTMB results and
  downstream analyses.

The corrected fixed-parameter bridge supports implementation equivalence for
the tested configuration. The historical bridge remains in the repository as
an explicit comparator. Alternative selectivity forms and diagnostic analyses
remain development products, while the accepted ADMB model remains the
...
The current RTMB implementation is an ADMB-port bridge within a development
workflow. It uses the corrected staged run in
`analysis/output/corrected_full_age_bts/admb_root/runs/full_age_bts/` for
initialization and comparison. This report reads the corrected RTMB output at
`analysis/output/corrected_full_age_bts/rtmb_base.rds`. The historical
`analysis/output/base.rds` provides a direct audit of the sample-size and
composition-normalization change.
...
ADMB output are resolved through `R/config.R` using the external pollock
workspace supplied by `POLLOCK_ROOT`.
...
| Time-age varying double logistic | Shared ascending and descending curve with annual random effects at a fixed 20% process CV; the tested formulation evaluates ages through 15 separately | Converged staged development fit |
...
[fishery-selectivity technical appendix](appendix_fishery_selectivity.html),
generated from `reporting/appendix_fishery_selectivity.qmd`.
...
The refreshed corrected artifact is the authoritative starting point for the
primary results below. Its configuration label, bridge statistics, and file
identity propagate into the OSA diagnostics, Only-BTS sensitivity,
retrospectives, and projections. The SparseNUTS appendix retains its separate
hierarchical Form-2 development lineage and states that distinction explicitly.
...
These values pass the repository's automated corrected-bridge tolerances.

```{r}
#| label: tbl-correction-effect
#| tbl-cap: "Effect of replacing the historical RTMB-adjusted BTS composition treatment with the corrected full-age treatment. Percent differences summarize paired annual values from the two saved RTMB bridge reports."
...
The RTMB bridge can now write a Standard Projection Model input directory for
use with `spmR` [@ianelli2026spmR]. This report writes the current saved RTMB
output to `r display_path(projection_dir)`, using the same SPM setup structure
as the ADMB bridge projection directory while replacing the species assessment
input file with an RTMB-derived `pm.prj`.
...
The installed `spmR` package (version `r spmr_version`) is attached by the reproducible projection script
...
The corrected projection directory is a reproducible input handoff. The rebuild
script executes the production ADMB engine through `spmR::runSPM()` and records
the corrected RTMB base checksum in `base_lineage.csv`. The current experimental
RTMB projection engine remains available for implementation testing.
...
index is a biomass index. The corrected bridge provides a practical platform for
diagnostics and model development, while the historical artifact provides an
explicit comparison of the likelihood treatment.

The OSA diagnostics, Only-BTS sensitivity, nine retrospective peels, and spmR
projections all carry the corrected base checksum. The retrospective workflow
preserves stream-specific data lags and fixes terminal-year selectivity at the
penultimate-year curve. Positive Mohn's rho values identify a stability issue
for continued evaluation. Candidate refinements require simulation testing and
review for operational use. ADMB retains its role as the accepted management
model during this development process, and the hierarchical Form-2 SparseNUTS
appendix remains a separate development lineage.
...
This analysis applies the current `SparseNUTS` package to the **hierarchical
Form-2 double-logistic development model** (`fishery_sel_form = 2`). The
accepted Form-0 ADMB bridge remains the management reference. The Form-2
...
object lets the package rebuild the objective in its parallel R workers, so the
package-default chains can run concurrently.

The completed fit is saved outside the rendered document. Every render reads
that fit and rebuilds all MCMC tables and figures, preventing diagnostics from
an older run from remaining in the report. The reproducible runner is
`R/run_sparsenuts_fishery_sel_forms.R`.
...
This report supports model-port debugging and bridge diagnostics. The parent
cross-platform comparison documents in `doc/` remain the broader comparison
record, and operational assessment use requires the full set of convergence,
estimability, retrospective, sensitivity, simulation, and review gates.
